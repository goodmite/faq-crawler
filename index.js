  ...
  let [questionsFound, answerObj] = await page.evaluate(async () => {
    ...
    function removeNodesByNames(mainElement, nodeNames) {
      nodeNames.forEach((node) => {
        mainElement
          .querySelectorAll(node.toLowerCase())
    ...
    function getHTMLBetweenNodes(node1, node2) {
      ...
      removeNodesByNames(tempDiv, ["BUTTON", "IMG", "iframe"]);
    ...
    function removeTagsExceptWhiteListed(htmlString = "", whitelist = []) {
      ...
      return htmlString
        .replace(new RegExp(`<${whitelistPattern}\/?.*?>`, "g"), "")
        .trim();
    }

    // select all text nodes with question mark with parents div, span, a, strong,

    const textNodes = findTextNodesWithQuestionMark(
    ...
    for (let i = 0; i < textNodes.length - 1; ++i) {
      res.push(
        getHTMLBetweenNodes(
          textNodes[i].parentNode,
          textNodes[i + 1].parentNode,
        ),
      );
    }
    ...
    const final = textNodes.map((e, i) => {
      return {
        ...
        plain: removeMultipleSpaces(
          removeTagsExceptWhiteListed(res[i]?.textContent, []),
        ),
      };
    });
    console.log(final);
